...
async def run(config_path: str, *, test_order: bool) -> int:
    ...
    try:
        ...
        account, open_orders = await asyncio.gather(
            client.account(), client.open_orders(instrument.symbol)
        )
        ...
        result["signed"] = {
            "ok": bool(account.get("canTrade", False)),
        ...
        print(json.dumps(result, indent=2))
        return 0 if account.get("canTrade", False) else 1
    finally:
        await client.close()
