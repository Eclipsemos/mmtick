...
test('paper console renders live state and operational views', async ({ page }) => {
  ...
  await expect(strategyParameters.getByText('28', { exact: true })).toBeVisible()
  await expect(strategyParameters.getByText('ATR 倍数')).toBeVisible()
  await expect(strategyParameters.getByText('1.25', { exact: true })).toBeVisible()
  ...
  await expect(strategyParameters.getByText('先平仓，再确认')).toBeVisible()
  await expect(strategyParameters.getByText('1x isolated')).toBeVisible()
...
test('price chart renders ATR line and trade percentage markers', async ({ page }) => {
  ...
  await expect(recentTrades.getByText('做多')).toHaveCount(2)
  await expect(recentTrades.getByText('做空')).toHaveCount(1)
  await expect(recentTrades.getByText('BUY -> SELL')).toHaveCount(2)
  await expect(recentTrades.getByText('SELL -> BUY')).toHaveCount(1)
