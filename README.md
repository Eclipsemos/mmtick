...
可直接用仓库中的 `agg_trades` 做无未来数据的 Tick 级参数网格回放：

```bash
PYTHONPATH=src /home/spaceaic/env/.venv/bin/python -m mastermind_tick.backtest
```

默认比较 ATR 周期 `5, 7, 10, 14, 21, 28, 35, 42, 56` 与倍数
`0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 2.5, 3.0`。回放严格沿用当前
Tick 穿越、同 K 线交易锁和下一 Tick 成交语义；现货与永续分别使用各自配置的
Taker 手续费、滑点、仓位比例和杠杆，永续同时按历史费率与模拟持仓重新计算资金费。
结果写入 `reports/` 下的 JSON 与 Markdown 文件，且不会修改模拟盘数据库。
